...
export const salesOrderController = {
  ...
  /**
   * Get sales orders for the authenticated customer
   */
  getCustomerOrders: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await salesOrderService.getOrdersByCustomerId({
        customerId,
        page,
        limit,
      });
      ...
      res.status(200).json({
        success: true,
        message: 'Customer sales orders retrieved successfully',
        data: result.data,
        pagination: result.pagination,
      });
    } catch (error) {
  ...
  getOrderById: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const order = await salesOrderService.getOrderById(orderId, userId);
